...
- 新增备课模式与授课模式、分步显隐及步骤与对象联动；有教学意义的动态参数默认启用，无需额外确认。
- 对象树统一按点、线、面、体分类，过滤文本和内部计算对象；对象树、搜索与属性栏使用一致的教师可读名称。
...
7. 题目包含动点或参数时，画布下方会自动显示动态滑动条，无需额外启用。
8. 使用按“点、线、面、体”分类的对象树搜索对象，并调整标签、显隐、颜色、线宽和拖动状态。
9. 进入授课模式可按构造步骤逐步展示图形；使用画布右上角下载菜单可导出 `.ggb` 或独立网页版 `.html`。
